...
Ported from shadow-perihelion/scripts/youtube-worker/youtube_fetch.py (2026-05-11).
Key change: fetch_metadata_ytdlp() → fetch_metadata_api_v3() using YOUTUBE_API_KEY.
Oracle Cloud VPS is IP-banned by YouTube — yt-dlp metadata fails. API v3 is the fix.

Dependencies (already in requirements.txt):
  youtube-transcript-api>=1.2.4
  yt-dlp>=2024.1.0  (transcript fallback only)

Environment variables:
  YOUTUBE_API_KEY  — Google Data API v3 key (GCP: glass-turbine-388620)
                     Required for metadata on VPS. Optional locally (yt-dlp fallback).

Author: vse-strateg-01 | video-seo-engine | 2026-06-15
"""
import json
import logging
import os
...
def format_published_date(raw: str) -> str:
    ...
    if not raw:
        return ""
    try:
        # Already ISO 8601 (from API v3: '2026-01-15T10:00:00+00:00')
        if 'T' in raw:
            return raw
...
def fetch_transcript_ytdlp(
    ...
    Note: May fail on Oracle Cloud VPS due to YouTube IP ban.
    Used as last resort when transcript-api also fails.
    """
...
# ---------------------------------------------------------------------------
# Metadata fetching — YouTube Data API v3 (primary, works on Oracle Cloud VPS)
# ---------------------------------------------------------------------------

def fetch_metadata_api_v3(video_id: str, api_key: str) -> dict:
    """Fetch video metadata via YouTube Data API v3.

    Does NOT make direct TCP connections to YouTube video servers — uses
    googleapis.com which is NOT blocked on Oracle Cloud VPS.
    ...
    try:
        ...
        duration_iso_str = item["contentDetails"]["duration"]  # already ISO 8601
...
# ---------------------------------------------------------------------------
# Metadata fallback — yt-dlp (local only, fails on VPS due to IP ban)
# ---------------------------------------------------------------------------
...
def fetch_metadata_ytdlp(video_id: str) -> dict:
    ...
    or as last-resort fallback when YOUTUBE_API_KEY is not set.
    """
    url = f"https://www.youtube.com/watch?v={video_id}"
    try:
...
def process_video(video_id: str, output_dir: str, lang: str = "pl") -> dict:
    ...
    if not vtt_text:
        logger.info("[fetcher] transcript-api failed, trying yt-dlp fallback...")
